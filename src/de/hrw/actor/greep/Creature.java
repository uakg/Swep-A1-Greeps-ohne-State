...
public abstract class Creature extends Actor {
	...
	/**
	 * Move forward roughly in the current direction. Sometimes we get a little
	 * off course.
	 */
	public void move() {
	...
	/**
	 * To avoid confusion/cheating, calling the built-in Greenfoot move(int
	 * amount) does the same as the Creature move() function.
	 */
	...
	/**
	 * Load a tomato onto *another* creature. This works only if there is
	 * another creature and a tomato pile present, otherwise this method does
	 * nothing.
	 */
	public final void loadTomato() {
		...
		if (greep != null && tomatoes != null) {
			// We have found a buddy greep
			if (!greep.carryingTomato()) {
				// .... and he is not carrying any tomatoes ;-)
				tomatoes.takeOne();
				greep.carryTomato();
			}
		}
	...
	/**
	 * Remove the tomato currently carried (and return true). Return false if we
	...
	/**
	 * Drop the tomato we are carrying. If we are at the ship, it is counted. If
	 * not, it's just gone...
	 */
	...
	/**
	 * Test if we are close to one of the edges of the world. Return true if we
	 * are.
	 */
	...
	/**
	 * Spit a drop of paint onto the ground. We can spit in three colors: "red",
	 * "orange", and "purple". (All other strings will be mapped to one of
	 * these.)
	 */
	...
	/**
	 * Store a user defined value. Attention: even though the parameter type is
	 * int, only byte size values (0 <= val <= 255) are accepted.
	 */
